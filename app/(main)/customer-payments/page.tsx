...
import {
  deletePaiementClient,
  exportPaiementsClientExcel,
  getPaiementsClient,
  type PaiementClientRow,
} from '@/lib/api/paiement-client';
...
const columns: Column<PaiementClientRow>[] = [
  {
    key: 'id',
    label: 'ID',
    sortable: true,
...
export default function CustomerPaymentsPage() {
  ...
  const [rows, setRows] = useState<PaiementClientRow[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const resp = await getPaiementsClient({
        pageNumber: 1,
        pageSize: 50,
        clientId,
        accountingYearIds:
  ...
  }, [clientId, accountingYearId, dateEcheanceFrom, dateEcheanceTo, montantMin, montantMax]);
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter(
      (r) =>
        ...
        (r.clientNom ?? '').toLowerCase().includes(t) ||
  ...
  const tableColumns = useMemo(() => {
    const actionCol: Column<PaiementClientRow> = {
      id: 'actions',
      key: 'id',
      label: 'Actions',
      sortable: false,
      ...
      render: (_v, row) => (
        <Button
          ...
          onClick={async () => {
            if (!window.confirm(`Delete client payment #${row.id}?`)) return;
            try {
              await deletePaiementClient(row.id);
              await load();
            } catch (e: unknown) {
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="space-y-1">
          <h1 className="text-4xl font-bold text-foreground">Customer payments</h1>
          <p className="text-muted-foreground">Client receipts (paiement client), filters and Excel export.</p>
        </div>
        <div className="flex gap-2">
          <Button
            ...
            onClick={async () => {
              try {
                const mMin = montantMin.trim() === '' ? null : Number(montantMin);
                const mMax = montantMax.trim() === '' ? null : Number(montantMax);
                const blob = await exportPaiementsClientExcel({
                  clientId,
                  accountingYearIds:
                    accountingYearId !== 'all' ? [Number(accountingYearId)].filter((n) => Number.isFinite(n)) : null,
                  dateEcheanceFrom: dateEcheanceFrom || null,
                ...
                downloadBlob(blob, 'PaiementsClient.xlsx');
              } catch (e: unknown) {
          ...
          <Button variant="outline" className="gap-2 opacity-60" disabled title="PDF export is not available on the API yet">
      ...
      <Card className="p-4 border-border">
        <div className="flex flex-wrap gap-4 items-end">
          <div className="flex-1 min-w-[220px] space-y-1">
            ...
            <CustomerCombobox value={clientId} onChange={(id) => setClientId(id)} placeholder="All customers" />
          </div>
      ...
      <DataTable
        ...
        searchPlaceholder="Search id, customer, reference, method..."
